...
export default function Hobbies({ hobbies }: HobbiesProps) {
  const defaultHobbies: HobbyItem[] = [
    { name: "编程", icon: "code" },
    { name: "阅读", icon: "menu_book" },
    { name: "跑步", icon: "directions_run" },
    { name: "音乐", icon: "music_note" }
  ];

  const hobbiesData = hobbies && hobbies.length > 0 ? hobbies : defaultHobbies;
